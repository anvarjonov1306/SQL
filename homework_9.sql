...
select Departments.DepartmentID , Employees.EmployeeID  from Departments cross join Employees
...
select Products.ProductName,Orders.OrderID from Products inner join Orders on Products.Productid=Orders.ProductID

---Using Departments, Employees table List employees whose DepartmentID matches the department.

select Departments.Departmentname, Employees.name from Employees inner join Departments on Departments.DepartmentID=Employees.DepartmentID
...
select Students.Name, Enrollments.StudentID from Students inner join Enrollments on Students.StudentID=Enrollments.StudentID
...
select Payments.PaymentID, Orders.OrderID from Payments inner join Orders on Payments.OrderId= orders.OrderID

----Using Orders, Products table Show orders where product price is more than 100

select orders.OrderID, orders.ProductID, Orders.OrderDate, orders.Quantity,
products.Price, Products.ProductName from orders inner join  Products on Orders.ProductID=Products.ProductID where price >100
...
select Employees.Name, Departments.DepartmentName from Employees cross join Departments where Employees.EmployeeID<>Departments.DepartmentID

---Using Orders, Products table Show orders where ordered quantity is greater than stock quantity.

select orders.ProductID, orders.OrderID, orders.Quantity,Products.ProductName, Products.StockQuantity from Orders inner join Products
on orders.ProductID=Products.ProductID where Orders.Quantity>Products.StockQuantity
...
select Customers.FirstName,sales.CustomerID from Customers inner join Sales on Customers.CustomerID=Sales.CustomerID where SaleAmount>=500
...
select products.ProductName,Suppliers.SupplierName from Products inner join Suppliers on Products.ProductID=Suppliers.SupplierID
where SupplierName like '%tech%'


---Using Orders, Payments table Show orders where payment amount is less than total amount.

select Orders.OrderID,Orders.TotalAmount, Payments.Amount from Orders inner join Payments on Orders.OrderID=Payments.OrderID 
...
select Products.ProductName,Categories.CategoryName from Products inner join Categories on Products.Category=Categories.CategoryID
...
select sales.CustomerID,Customers.Country, Customers.FirstName from Sales inner join  Customers on sales.CustomerID=Customers.CustomerID
where Country='USA'

---Using Orders, Customers table List orders made by customers from 'Germany' and order total > 100.

select Orders.OrderID,Customers.FirstName, Customers.Country from Orders inner join Customers on Orders.CustomerID=Customers.CustomerID
...
select e1.EmployeeID as Employee1_id, e1.name as Employee1_name, e2.EmployeeID as employee2_id,
e2.Name as employee2_name from Employees e1 join Employees e2 on e1.DepartmentID<>e2.DepartmentID 
...
select payments.PaymentID,Payments.OrderID,Payments.Amount,Orders.Quantity,products.Price from payments  join orders on Payments.OrderID=Orders.OrderID
 join Products on Payments.OrderID=Products.ProductID
...
---Using Employees table List employees who are managers of someone, but their salary is less than or equal to the person they manager.

select e1.employeeid as employee1_id, e1.name as employee1_name, e2.employeeid as employee2_id, e2.name as employee2_name
from Employees e1 join Employees e2 on e1.DepartmentID<>e2.DepartmentID 
where e1.EmployeeID<e2.EmployeeID

---Using Orders, Payments, Customers table List customers who have made an order, but no payment has been recorded for it.

select c.CustomerID,c.FirstName from Customers c join orders o on c.CustomerID=o.CustomerID left join payments p on 
o.OrderID=p.OrderID where p.paymentid is null
