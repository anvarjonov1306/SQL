...
select e.name,e.salary,d.departmentname from Employees e join Departments d on e.DepartmentID=d.DepartmentID
where e.Salary>5000
...
select s.SupplierName,p.ProductName from Suppliers s left join Products p on s.SupplierID=p.ProductID
...
select o.OrderID, o.OrderDate, p.PaymentDate, p.Amount from Orders o full outer join Payments p on o.OrderID=p.PaymentID
...
select e.name as employeename, m.name as managername from Employees e left join Employees m on e.EmployeeID=m.ManagerID
...
----Using the Customers and Orders tables, write a query to find customers who have placed an order with more than 3 items. Return their name and the quantity they ordered.
 ...
 select c.firstname, c.lastname,o.quantity from Customers c join Orders o on c.CustomerID=o.CustomerID
 where o.quantity>3

 ----Using the Employees and Departments tables, write a query to list employees working in the 'Human Resources' department

 select e.* from Employees e join Departments d on e.DepartmentID=d.Departmentid
...
select d.departmentname from Departments d join Employees e on d.Departmentid=e.DepartmentID
...
select p.ProductID,p.productname from  Products p left join sales s on p.productid=s.productid
...
select e.*, d.* from Employees e join Departments d on e.departmentid=d.DepartmentID
...
e2.name as employee2_name, e1.managerid from Employees e1 join Employees e2 on e1.ManagerID=e2.managerid
...
select e.employeeid, e.name, e.salary , d.departmentname from Employees e join Departments d on e.DepartmentID=d.DepartmentID
...
select o.orderid, o.orderdate,p.Paymentdate,p.amount from orders o join payments p on o.OrderID=p.OrderID
...
select e.name , e.employeeid, e.DepartmentID , e.salary from Employees e join (select Departmentid,avg(salary) as avg_salary from Employees
group by Departmentid) dept_avg on e.DepartmentID=dept_avg.DepartmentID where e.Salary>dept_avg.avg_salary
...
select p.productid,p.productname from products p left join Categories  c on c.categoryid=p.ProductID
where c.categoryid is null

---Using the Employees table, write a query to find employees who report to the same manager and earn more than 60000

select e1.employeeid, e1.salary,e1.managerid from Employees e1 join Employees e2 on e1.ManagerID=e2.ManagerID
...
select p.productname ,s.* from sales s join products p on s.ProductID=p.ProductID
...
select Students.Name, Courses.CourseName from Students left join Enrollments on Students.StudentID=Enrollments.StudentID 
...
select o.* from orders o  left join  Payments p  on  o.OrderID=p.OrderID where p.OrderID is null

----Using the Products and Categories tables, write a query to list products that belong to either the 'Electronics' or 'Furniture'

select p.* from  products p join Categories c on  p. productid=c.CategoryID
