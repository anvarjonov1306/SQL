...
select ltrim(rtrim(substring(name,1,charindex(',',name)-1))) as surname,
...
select regexp_replace('1234ABC123456XYZ1234567890ADS','\d','X','g') as replaced_string
...
select CountSpaces=len(texts)-len(replace(texts,' ','')) from CountSpaces
...
select e.id,e.name,e.salary,m.managerid,m.name as managername,
m.salary as managersalary from Employee e join  Employee m on e.ManagerId=m.Id
...
datediff(YEAR,hire_date,getdate()) - case when month (hire_date)>month(getdate()) or (month(hire_date)=month(getdate())
and day(hire_date)>day(getdate())) then 1 else 0 end as year_of_service
from Employees
datediff(YEAR,hire_date,getdate()) case when month(hire_date)>month(getdate()) or (month(hire_date)=month(getdate())
and day(hire_date)>day(getdate())) then 1 else 0 end between 11 nd 14
...
SELECT
    OriginalString,
    LEFT(OriginalString, PATINDEX('%[0-9]%', OriginalString + '0') - 1) AS CharacterPart,
    SUBSTRING(OriginalString, PATINDEX('%[0-9]%', OriginalString), LEN(OriginalString)) AS NumberPart FROM
( SELECT 'rtcfvty34redt' AS OriginalString) AS t
...
SELECT Id,RecordDate,Temperature FROM (SELECT Id, RecordDate,Temperature,
       LAG(Temperature) OVER (ORDER BY RecordDate) AS PreviousDayTemp
    FROM weather) AS sub
WHERE Temperature > PreviousDayTemp
...
WITH CharTable AS(SELECT 1 AS Position,
...
,(12,'111')
,(13,NULL)
SELECT Vals,CAST(SUBSTRING(Vals,1,PATINDEX('%[^0-9]%', Vals + 'X') - 1 ) AS INT) AS GetIntegers
FROM GetIntegers
WHERE Vals LIKE '[0-9]%'; 
...
SELECT Id,CONCAT(SUBSTRING(Vals, CHARINDEX(',', Vals) + 1, CHARINDEX(',', Vals + ',', CHARINDEX(',', Vals) + 1) - CHARINDEX(',', Vals) - 1),
        ',', SUBSTRING(Vals, 1, CHARINDEX(',', Vals) - 1),
        SUBSTRING(Vals, CHARINDEX(',', Vals, CHARINDEX(',', Vals) + 1), LEN(Vals))) AS SwappedVals
FROM MultipleVals;
...
FROM (SELECT Player_ID, Device_id,games_played,
...
WITH WeeklyAreaSales AS (
    SELECT
        ...
        FinancialYear,
        Area,
        SUM(SalesLocal + SalesRemote) AS AreaWeeklySales
...
WeeklyTotalSales AS (
    SELECT
        FinancialWeek,
        FinancialYear,
        SUM(SalesLocal + SalesRemote) AS TotalWeekSales
    FROM WeekPercentagePuzzle
    GROUP BY
        FinancialWeek,
        FinancialYear
...
DailyAreaSales AS (
    SELECT
        ...
        wpf.Area,
        wpf.[Date],
        SUM(wpf.SalesLocal + wpf.SalesRemote) AS AreaDaySales
    FROM WeekPercentagePuzzle wpf
    GROUP BY
        wpf.FinancialWeek,
        wpf.FinancialYear,
        wpf.Area,
...
SELECT
    ...
    d.FinancialWeek,
    d.FinancialYear,
    (d.AreaDaySales * 100.0) / tws.TotalWeekSales AS PercentageOfWeek
FROM DailyAreaSales d
JOIN WeeklyTotalSales tws
    ON d.FinancialWeek = tws.FinancialWeek
    AND d.FinancialYear = tws.FinancialYear
ORDER BY
    d.FinancialWeek,
    d.Area,
    d.[Date];