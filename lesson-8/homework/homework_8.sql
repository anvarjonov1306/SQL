...
select category,  sum(productid) as total_number from products
group by category
...
select *, case when   StockQuantity<30 then	'Low stock' else 'Sufficient' end from products 

----Using Customers table, find the total number of customers in each country.

select country , sum(customerid) as total_number from customers
group by country
...
select max(Quantity) as max_qty , min(Quantity) as min_qty from orders
...
FROM Orders o
LEFT JOIN Invoices i ON o.orderid = i.CustomerID
...
select orderdate , avg(quantity) as avg_qty from orders
group by orderdate
...
when price between 100 and 500 then 'mid' else 'high' end as pricegroup from products
...
select productid,sum(saleamount) as sum_amt from sales
group by ProductID
...
);INSERT INTO Population_Each_City (district_id, district_name, Bektemir, Chilonzor, Yakkasaroy)
SELECT
    district_id,
    district_name,
    [Bektemir],
...
(
    MAX(population)
    FOR ([Bektemir], [Chilonzor], [Yakkasaroy])
) AS PivotTable;
...
UNPIVOT
(
    Population FOR District IN (Bektemir, Chilonzor, Yakkasaroy)
) AS unpvt;
...
UNPIVOT
(
    Population FOR District IN (District1, District2, District3)
) AS unpvt;