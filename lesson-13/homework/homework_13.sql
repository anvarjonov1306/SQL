...
select concat(EMPLOYEE_ID,'-',first_name,' ',last_name) as emp_info from Employees
...
select first_name as firstname , len(first_name) as firstnamelegth from Employees
...
select id , coalesce (ssn,passportid,itin) as first_non_null_value from person
...
case when len(fullname)-len(replace(fullname,' ' , ''))>=2 then SUBSTRING(fullname, charindex(' ' ,fullname)+1, charindex(' ' , fullname,charindex(' ',fullname)+1)-charindex(' ',fullname)-1)
else null end as middlename,
case when charindex (' ',fullname) > 0 then right (fullname,len(fullname)-charindex(' ', fullname,charindex(' ' , fullname)+1))else null
end as lastname from Students
...
where o.DeliveryState='TX' and o.CustomerID in(select distinct CustomerID from orders 
...
group  by SequenceNumber
 ...
 select product, unitprice,EffectiveDate from products
 where unitprice>100
 ...
 case when count(*)=0 then 0 else (cast(count(case when datediff(year,hire_date,getdate())>3 then 1 end) as decimal(5,2))*100)/count(*)
  end as percentageover3years from employees
...
select JobDescription, SpacemanID, MissionCount, 'Most Experienced' as ExperienceLevel from Personal p 
where  MissionCount=(select max (MissionCount) from personal
where JobDescription = p.JobDescription
union all
select JobDescription, SpacemanID, MissionCount, 'Least Experienced' as ExperienceLevel from Personal p 
...
SELECT
  REGEXP_REPLACE(str, '[^A-Z]', '', 1, 1, 'c') AS uppercase_letters,
  REGEXP_REPLACE(str, '[^a-z]', '', 1, 1, 'c') AS lowercase_letters,
  REGEXP_REPLACE(str, '[^0-9]', '', 1, 1, 'c') AS numbers,
  REGEXP_REPLACE(str, '[A-Za-z0-9]', '', 1, 1, 'c') AS other_chars
FROM input_str
...
ALTER TABLE Equations ADD TotalSum INT NULL;
...
BEGIN
    SET @SQL = 'SELECT @Res = ' + @Equation
    EXEC sp_executesql @SQL, N'@Res INT OUTPUT', @Res=@Result OUTPUT;
    UPDATE Equations
    SET TotalSum = @Result
...
SELECT SUM(TotalSum) AS TotalSumOfAllEquations FROM Equations;
...
('Aaron', '2015-12-18');
GO 
SELECT StudentName, s.Birthday
FROM Student s
JOIN (SELECT Birthday FROM Student
GROUP BY Birthday
HAVING COUNT(*) > 1) dup ON s.Birthday = dup.Birthday
ORDER BY s.Birthday;
...
SELECT CASE WHEN PlayerA < PlayerB THEN PlayerA ELSE PlayerB END AS Player1,
