...
select sum( SaleAmount) as sales_amt from sales
where productid=7
...
---Write a query to calculate the average age of employees in the Employees table.
 ...
 select category, min(price) as min_pr, max(price) as max_prc from Products
 group by category
 ...
 select ProductID ,sum(saleamount) as sum_slm, avg(saleamount) as avg_slm from sales
 group by ProductID
 ...
 SELECT departmentname,employeeid,max(salary) as max_sly, min(salary) as min_sly from Employees
 group by departmentname,employeeid
...
select departmentname, avg(salary) as avg_slr, count(employeeid) as cnt_emp from Employees
group by departmentname
...
select departmentname, avg(salary) as avg_slr from Employees
group by departmentname
...
select customerid, sum(saleamount) as sum_slmt from sales
group by customerid
...
select departmentname, sum(salary) as sum_slr from Employees
group by departmentname
...
select customerid, sum(quantity) as total_amount_over_50 , min(quantity) as least_purchase
from orders
where quantity > 50
group by customerid
...
select year(orderdate) as year,month(orderdate) as month,sum(quantity) as total_sales,
count(DISTINCT productid) as unique_products_sold from orders
group by year(orderdate),month(orderdate)
 having count (distinct productid) >= 2;

 ---Write a query to find the MIN and MAX order quantity per Year. From orders table. 
 select orderdate, max(quantity) as max_qnt, min(quantity) as min_qnt from orders
 group by orderdate