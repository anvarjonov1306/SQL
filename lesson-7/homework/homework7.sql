...
--3
select count(customerid) as cust_id from customers
--4
select count(category) as pro_cat from Products
--5
select sum(SaleAmount) as sal_amt from sales
where productid=7
--6
select avg(age) as avg_age from Employees
--7
select departmentname,count(firstname) as employee_count from employees
...
having count(*)>5
--11
select sum( SaleAmount) as sale_amt,avg(SaleAmount) as avg_sale from sales
group by productid
--12
select count(departmentname) as count_depart from employees
where DepartmentName='HR'
--13
select DepartmentName, max(salary) as max_sal, min(salary) as min_sal from Employees
group by DepartmentName
--14
select DepartmentName,avg(salary) as avg_salary from Employees
group by DepartmentName
--15
select DepartmentName, avg(salary) as avg_salary, count(employeeid) as num_emp from Employees
group by DepartmentName
--16
select Category, avg(price) as price from products
where price>400
group by Category
--17
...
group by SaleDate
--18
select   customerId,count(*) as employee_count from Orders
group by customerId
having count(*)>=3
--19
select  departmentname , avg(salary) as avg_salary from Employees
...
--21
select customerid, sum(saleamount) as sal_amt from sales
group by CustomerID
...
--25
select year(orderdate) as order_year, max(Quantity) as max_qty, min(Quantity) as min_qty from orders
